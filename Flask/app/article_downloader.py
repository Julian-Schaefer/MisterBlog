...
def download_blog_posts(
    latest_date: datetime, blog_selections: List[BlogSelection]
) -> List[BlogPost]:
    ...
    if len(blog_selections) > 0:
        ...
        while True:
            ...
            candidate_blog_post_urls: List[
                Tuple[BlogSelection, List[str]]
            ] = urlPool.starmap(
                get_article_urls,
                [(page, blog_selection) for blog_selection in blog_selections],
            )
            urlPool.close()
            urlPool.join()
            ...
            for (blog_selection, urls) in candidate_blog_post_urls:
                ...
                if (
                    last_blog_post
                    and last_blog_post.date
                    and (not latest_date or last_blog_post.date < latest_date)
                ):
                ...
                for (blog_selection, urls) in blog_post_urls:
                    ...
                    for blog_post in current_blog_posts:
                        if (
                            blog_post
                            and blog_post.date
                            and (not latest_date or blog_post.date < latest_date)
                        ):
            ...
            if len(blog_posts) > 10:
                blog_posts.sort(key=lambda blog_post: blog_post.date, reverse=True)
...
def download_article(url: str, download_content: bool = False) -> BlogPost:
    try:
        ...
        trafilatura_doc = trafilatura.bare_extraction(
            html_doc,
        ...
        if not blogPost.date and trafilatura_doc["date"]:
            blogPost.date = datetime.strptime(trafilatura_doc["date"], "%Y-%m-%d")

        blogPost.summary = (
            " ".join(trafilatura_doc["raw_text"].split(" ")[:100]) + "..."
        )
        ...
        if download_content:
            ...
            if (
                not simple_article["plain_text"]
                or len(simple_article["plain_text"]) == 0
            ):
