...
def get_rss_url(blog_url: str) -> Tuple[str, bool]:
    ...
    rss_urls = [
        rss_link["href"]
        for rss_link in page_soup.find_all("link", {"type": "application/rss+xml"})
    ]
    ...
    for rss_url in rss_urls:
        ...
        if rss_feed and rss_feed["entries"] and len(rss_feed["entries"]) > 0:
            number_of_contained_entries = [
                ...
                if entry["title"] in normalized_html_content
            ]
    ...
    for page in pages:
        ...
        if (
            not first_page
            or not first_page["entries"]
            or len(first_page["entries"]) == 0
        ):
            break
        ...
        if (
            not second_page
            or not second_page["entries"]
            or len(second_page["entries"]) == 0
        ):
            break

        if first_page["entries"][0]["title"] != second_page["entries"][0]["title"]:
            # Methode 2: Only RSS can be used, fuck yeah!!!!
            # --> schreibe "rss_url" in DB und benutze zum holen von Articles
            paginated = True
            return (valid_rss_url, paginated)
        else:
            # Methode 3: Find Article Selector nicht so aufwändig in HTML using
            # titles of first RSS Page
            paginated = False
            return (valid_rss_url, paginated)
